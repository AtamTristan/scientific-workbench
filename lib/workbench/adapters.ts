...
export const repositoryAdapters: RepositoryAdapter[] = repositoryAdapterSchema.array().parse([
  {
    ...
    requiredFiles: [".scientific/repository.json", "thesis/validate.py", "thesis/run.py"],
    environment: { type: "conda", name: "hypergraph-ba" },
    validateCommand: ["python", "-m", "thesis.validate"],
    runCommand: ["python", "-m", "thesis.run", "--plan", "{plan}"],
    outputPatterns: ["reports/runs/*.json", "reports/**/*.json"],
    notes: [
      "The repository descriptor is authoritative; the adapter blocks until it is present.",
      "A generated ExperimentPlan carries dataset, strategy, rho, seed and provenance expectations.",
    ],
  },
  {
    schemaVersion: "1.0",
